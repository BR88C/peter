...
export default {
    ...
    exec: async (ctx) => {
        ...
        if (!player || player.state < PlayerState.CONNECTED) return void ctx.error(`Unable to pause the music; the bot is not connected to a VC.`);
        ...
        if (foundVoiceState?.channel_id !== player.options.voiceChannelId) return void ctx.error(`You must be in the VC to pause the music.`);
