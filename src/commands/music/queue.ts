...
export default {
    ...
    exec: async (ctx) => {
        const voiceChannel = await ctx.worker.api.channels.get(ctx.player!.options.voiceChannelId).catch((error) => Utils.logError(error));
        ...
        const sendEmbed = async (page: number): Promise<void> => {
            ...
            ctx.player!.queue.forEach((track, i) => trackTitles.push(`${i === ctx.player!.queuePosition ? `↳ ` : ``}**${i + 1}.** ${(track as Track).uri ? `[` : ``}${Utils.cleanseMarkdown(track.title)}${(track as Track).uri ? `](${(track as Track).uri})` : ``} ${track.length ? ` - [${Utils.timestamp(track.length)}]` : ``}`));
            const pages: string[] = [];
            ...
            await ctx.embed
                ...
                .title(ctx.player!.currentTrack ? `**Now Playing:** ${ctx.player!.currentTrack.title} ${ctx.player!.currentTrack.length ? `[${Utils.timestamp(trackTimeLeft)} remaining]` : ``}` : `**No music playing**`)
                ...
                .field(`Queue Length`, `\`${Utils.timestamp(queueLength)}\``, true)
                .field(`Time Left`, `\`${ctx.player!.queuePosition !== null ? Utils.timestamp(queueLength - (ctx.player!.queue.slice(0, ctx.player!.queuePosition).reduce((p, c) => p + (c.length ?? 0), 0) + (ctx.player!.position ?? 0))) : `N/A`}\``, true)
