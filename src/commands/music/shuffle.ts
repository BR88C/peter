...
export default {
    ...
    exec: async (ctx) => {
        ...
        if (!player || player.state < PlayerState.CONNECTED) return void ctx.error(`Unable to shuffle; the bot is not connected to the VC.`);
        if (!player.queue.length) return void ctx.error(`Unable to shuffle; there are no tracks in the queue.`);
