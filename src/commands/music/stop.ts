...
export default {
    ...
    exec: async (ctx) => {
        ...
        if (!player || player.state < PlayerState.CONNECTED) return void ctx.error(`Unable to stop the music; the bot is not connected to a voice channel.`);
        ...
        if (foundVoiceState?.channel_id !== player.options.voiceChannelId) return void ctx.error(`You must be in the voice channel to stop the music.`);
