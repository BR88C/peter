        ...
        if (ctx.options.dj) {
            if (ctx.options.dj.command || typeof ctx.options.dj.useroverride === `number`) {
                ...
                if (ctx.options.dj.command) {
                    ...
                    if (!ctx.worker.commands.commands.find((command) => command.category === `music` && command.interaction!.name.toLowerCase() === ctx.options.dj.command.toLowerCase())) return void ctx.error(`The specified command does not exist.`);
                }
                if (ctx.options.dj.useroverride < 0) return void ctx.error(`Please specify a postivie value or 0 for the "useroverride" field.`);
                ...
                try {
                    ...
                    if (typeof ctx.options.dj.useroverride === `number`) newSettings.push(`Set the amount of users needed in a voice channel to restrict commands to DJ only to \`${ctx.options.dj.useroverride}\``);
                    ctx.embed
                ...
                if (!ctx.worker.commands.commands) return void ctx.error(`Unable to get the command list. Please try again.`);
                ctx.worker.mongoClient.db(Config.mongo.dbName).collection(`Guilds`).findOne({ id: ctx.interaction.guild_id })
                    .then((guildDocument) => {
