        ...
        if (ctx.options.dj) {
            if (ctx.options.dj.command || typeof ctx.options.dj.useroverride === `number`) {
                ...
                if (ctx.options.dj.command) {
                    ...
                    if (!ctx.worker.commands.commands.find((command) => command.category === `music` && command.interaction!.name.toLowerCase() === ctx.options.dj.command.toLowerCase())) return void ctx.error(`That command does not exist.`);
                }
