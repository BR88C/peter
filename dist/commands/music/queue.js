...
exports.default = {
    ...
    exec: async (ctx) => {
        ...
        const sendEmbed = async (page) => {
            const trackTitles = [];
            ctx.player.queue.forEach((track, i) => trackTitles.push(`${i === ctx.player.queuePosition ? `↳ ` : ``}**${i + 1}.** ${track.uri ? `[` : ``}${StringUtils_1.cleanseMarkdown(track.title)}${track.uri ? `](${track.uri})` : ``} ${track.length ? ` - [${Time_1.timestamp(track.length)}]` : ``}`));
            const pages = [];
            ...
            await ctx.embed
                ...
                .title(ctx.player.currentTrack ? `**Now Playing:** ${ctx.player.currentTrack.title} ${ctx.player.currentTrack.length ? `[${Time_1.timestamp(trackTimeLeft)} remaining]` : ``}` : `**No music playing**`)
                ...
                .field(`Time Left`, `\`${ctx.player.queuePosition !== null ? Time_1.timestamp(queueLength - (ctx.player.queue.slice(0, ctx.player.queuePosition).reduce((p, c) => p + (c.length ?? 0), 0) + (ctx.player.position ?? 0))) : `N/A`}\``, true)
