...
exports.default = {
    ...
    exec: async (ctx) => {
        ...
        if (!player || player.state < lavalink_1.PlayerState.CONNECTED)
            return void ctx.error(`Unable to shuffle; the bot is not connected to a voice channel.`);
        ...
        if (foundVoiceState?.channel_id !== player.options.voiceChannelId)
            return void ctx.error(`You must be in the voice channel to shuffle the queue.`);
