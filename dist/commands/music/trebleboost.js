...
exports.default = new cmd_1.ChatCommand()
    ...
    const newFilters = {
        ...player.filters,
        equalizer: (player.filters.equalizer?.filter((v) => v.band < Constants_1.Constants.EQ_BAND_COUNT - 3) ?? []).concat(ctx.parameters.value === 0 ? [] : new Array(3).fill(null).map((_, i) => ({
            band: Constants_1.Constants.EQ_BAND_COUNT - (i + 1),
            gain: ctx.parameters.value * Constants_1.Constants.TREBLE_INTENSITY_MULTIPLIER
        })))
    };
