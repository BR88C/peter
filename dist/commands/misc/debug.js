...
exports.default = {
    ...
    exec: async (ctx) => {
        if (ctx.options.permissions) {
            ...
            ctx.embed
                .color(Constants_1.default.DEBUG_EMBED_COLOR)
            ...
            const member = await ctx.worker.api.members.get(ctx.interaction.guild_id, ctx.worker.user.id).catch((error) => discord_utils_1.Utils.logError(error));
            ...
            const debugEmbed = new discord_rose_1.Embed()
                .color(Constants_1.default.DEBUG_EMBED_COLOR)
                .title(`Debug`)
                .description(`Peter's support server: ${Config_1.default.supportServer}\n**Thread ID:** \`${ctx.worker.comms.id}\``)
            ...
            if (player) {
                const playerTextChannel = await ctx.worker.api.channels.get(player.options.textChannelId).catch((error) => discord_utils_1.Utils.logError(error));
                ...
                const playerVoiceChannel = await ctx.worker.api.channels.get(player.options.voiceChannelId).catch((error) => discord_utils_1.Utils.logError(error));
